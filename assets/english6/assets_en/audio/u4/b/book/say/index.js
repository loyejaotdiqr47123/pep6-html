...
document.getElementById('dot11').addEventListener('click', function (event) {
  mouse_click(currentStart, event)
})


document.getElementById('dot12').addEventListener('click', function (event) {
  mouse_click(currentStart, event)
})
...
function mouseStart(event, id) {
  ...
  if (imgArr.indexOf(id) == -1) {
    ...
    mosue_start_y = event.target.offsetTop + (event.target.offsetHeight / 1.5)
    mouse_pos_x = mouse_start_x;
    mouse_pos_y = mosue_start_y;
