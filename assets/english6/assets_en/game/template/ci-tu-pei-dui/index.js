var data = ['first floor', 'second floor', 'teachers office', 'library', ]
                ...
                for (var i = 0; i < checkList.children.length; i++) {
                    ...
                    if (i === 0) {
                        scope.push({
                            ...
                            x2: checkList.children[i].offsetLeft + checkList.children[i].offsetWidth - 10,
    ...
    numberList[i].addEventListener('mouseup', function(e) {
        if (e.target.parentNode.nodeName === 'DIV') {
            ...
            for (var i = 0; i < scope.length; i++) {
                // 判断是否拖入方框
                if ((x1 >= scope[i].x1 && x2 <= scope[i].x2 && y1 >= scope[i].y1 && y2 <= scope[i].y2) || activeLi.target.getAttribute('data-flag') == 'classroom') {
                    // 打断前一次播放
                    // 是否正确
                    if (e.target.getAttribute('data-flag') == checkList.children[i].getAttribute('data-flag')) {
                        ...
                        break;
                    } else if (activeLi.target.getAttribute('data-flag') == 'classroom') { // 教室单独判断
                        // console.log('正确')
                        audioCorrect.currentTime = 0
                        audioCorrect.play()
                        correct += 1
                        totalPoints += grade
                        checkList.children[2].appendChild(e.target)
                        activeLi.target.style = ''
                        if (correct >= (data.length - 1)) {
                            clearInterval(countDown)
                            overBox.style.display = 'block'
                            overTime.innerText = '时间：' + totalTime
                            HP == 3 ? totalPoints = 100 : totalPoints = totalPoints.toFixed(0)
